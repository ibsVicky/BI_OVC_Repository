...
"""********************************************************************"""
def object_validation_error_insert(df_errors):
    try:
        ...
        query += "INSERT INTO `latam-md-finance`.control.object_validation_error (fk_object_validation_id, object_validation_error_value) VALUES"
        ...
        for row in err_rows:
            ...
            if first:
                query += f"({rejection_reason}, '{error_value}')"
                first = False
            else:
                query += f" ,({rejection_reason}, '{error_value}')"
        ...
        print(f"❌ ERROR INSERTING IN control.object_validation_error: {e}")
    return False
        ...
        if df_data.filter(~all_pass_condition).count() > 0:
            ...
            try:
                for df in rejected_records: #use for, because rejected_records is a list of dataframes
                    object_validation_error_insert(df)
            except Exception as e:
...
def business_validation(df_data: DataFrame, df_validations: DataFrame):
    ...
    for validation in list_validations:
        ...
        df_invalid =( src_data.join(
            ...
            .withColumn("error_value", F.concat_ws("||", *[F.col(src_col) for src_col, _ in rule_detail])) #these are the key values that failed
        )
...
def business_validation_bkp(df_data: DataFrame, df_validations: DataFrame):
    #Esta función valida y mueve a rejected, pero no inserta en log error table
    # Replace NULL with "UNSP" to avoid null issues
    src_data = df_data.fillna("UNSP")
    rejected_records = []
...
"""********************************************************************"""
def physical_validation_bkp(df_data, df_validations):
    #Esta función valida y mueve a rejected, pero no inserta en log error table
    # Validations who reject all the extract
    df_validations_reject_all = df_validations.filter(F.col("reject_all") == 1).collect()
